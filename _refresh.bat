...
IF EXIST "%input_cache_path%\%tooling_jar%" (
	ECHO running: JAVA -jar "%input_cache_path%\%tooling_jar%" -RefreshIG -ip=%~dp0 -igrp="%ig_resource_path%" -iv=fhir4 -t -d -p -v
	JAVA -jar "%input_cache_path%\%tooling_jar%" -RefreshIG -ip=%~dp0 -igrp="%ig_resource_path%" -iv=fhir4 -t -d -p -v %fsoption%
) ELSE If exist "..\%tooling_jar%" (
	ECHO running: JAVA -jar "..\%tooling_jar%" -RefreshIG -ip=%~dp0 -igrp="%ig_resource_path%" -iv=fhir4 -t -d -p -v
	JAVA -jar "..\%tooling_jar%" -RefreshIG -ip=%~dp0 -igrp="%ig_resource_path%" -iv=fhir4 -t -d -p -v %fsoption%
) ELSE (
